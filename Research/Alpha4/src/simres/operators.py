...
# 根据mask的True和False将alpha中对应位置为false的值置为NAN【筛选中证500股票】
...
# 衡量股票过去一段时间收益（或特征）分布的非对称性
# ts_skewness(alpha,window=3)
# Skew > 0（正偏/右偏）：说明过去 3 天里有某一天出现了极端的大暴涨（暴涨长尾）。
# Skew < 0（负偏/左偏）：说明过去 3 天里有某一天出现了极端的大暴跌（暴跌长尾）。
...
# 滚动峰度——衡量分布的尖锐程度和尾部厚度（极端异常值出现的概率）
# Kurt > 0：说明最近有极其罕见的“黑天鹅”事件或大妖股走势（肥尾）。【检测异常极端值】
# Kurt < 0：说明走势非常温和，两头无极端值。
# ts_kurtosis(alpha,window=4):
...
# 向前填充
# 让数据沿着 时间轴 向右画直线复制，直到遇到有效数据停止；
# ts_fill(alpha,window=2) 表示 最多帮你在空窗期复制2天，超过2天，那么就算后面数据还是为空的，也不会复制，会继续留空NaN
...
# ts_min 和 ts_max(alpha,window=3)——滚动历史极值
# 核心作用：分别找出过去3天内的历史最低分和历史最高分；
# 输出的也是一个 numpy矩阵，没有值的地方就是NAN
...
# 滚动时序标准化
# 用ts_mean(arr,window) / ts_std(arr,window)
def ts_zscore(arr,window):
    return ts_mean(arr,window)/ts_std(arr,window)

# 数据平移
# shift = 2 ，将arr整体向右推两格 （np.roll）
# 由于传送带滚动，于是用result[:,:shift]=np.nan 将第一天和第二天刷成NaN
...
# 求历史差值；
# ts——delta(arr,shift)，用今天的值，减去shift天前的值，计算出shift天内增加或减少的绝对值
def ts_delta(arr,shift):
    return arr-ts_delay(arr,shift)
# 求涨跌幅
# 用ts_delta(这个变化的差值)/ts_delay(arr,shift)(shift天前的值）————> 得出收益率
def ts_ret(arr,shift):
    return ts_delta(arr,shift)/ts_delay(arr,shift)
# 求滚动时序相关性
# ts_correlation(x,y,window=3)
# 表示 矩阵X 和 矩阵Y 在过去Window天的相似性——————（corr越接近1越正相关；corr越接近-1越负相关；corr越接近0越没关系）
# 这里的X Y一般都是用收益率的矩阵去比较的 ——————（X可以代表alpha01的收益率矩阵；Y可以代表alpha02的收益率矩阵）
...
# cs_zscore(alpha) —— 截面标准化（消灭量纲与极值）
# 按照date截面进行标准化
# 1. 求出截面的mean和std，然后用标准化公式去做即可
def cs_zscore(alpha):
    return (alpha-np.nanmean(alpha,axis=0))/np.nanstd(alpha,axis=0)
# 2. 资金仓位归一化（多空对冲中性化）
# 在同一天内：挑出因子分>0的股票(多头),按比例瓜分，多头的总仓位相加刚好为1
# 挑出因子分<0的股票(空头),按比例瓜分，空头的总仓位相加刚好为-1

# 多头阵营，按照 个股score/总score 去分配仓位，因子得分越大，那么我的仓位相应就越高
# 输出：最终输出的是一个真实的实盘交易权重numpy矩阵; 跟alpha因子是相对应的;
...
def cs_indneut(alpha, ind_matrix):
    ...
    # mask就是看看alpha或者ind_matrix的数据哪有NaN
    ...
    max_ind = int(np.nanmax(ind_matrix)) + 1  # 最大行业ID + 1
    offset = np.arange(n_dates) * max_ind # [0,2674] x max_ind
    flat_ind = (clean_ind + offset[None, :]).flatten() # clean_ind矩阵加上行向量offset[None,:]，即[[0,3]]
...
# 最终返回的是一个 已经是 截面排好序的numpy矩阵了！【含有NaN】
def cs_rank(alpha):
    """
...
# 经过cs_rank矩阵排名后，对每天进行分组，分为n_per_group组
# 直接用因子分数 / n_per_group ————> 得到对应的组号，组号越大，说明因子分越大，是属于越做多的那一组
# 最后还要mask过滤，将停牌股的 组号变成nan
